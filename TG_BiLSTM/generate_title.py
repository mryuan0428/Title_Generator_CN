...
title_bilstm_path = '/'.join(sys.path[0].split('/')[0:-1]) + '/validation/title_bilstm.txt'

cnt = 0
title = []
for line in open(content_path,"r",encoding="utf-8"):
    title_gen = gen_sent(line)
    title.append(title_gen)
    cnt += 1
    if cnt % 50 == 0:
        print(f"{cnt} title have been generated by bilstm_model.")

f = open(title_bilstm_path,'a+',encoding='utf-8')
for item in title:
    f.write(item+'\n')
f.close()
